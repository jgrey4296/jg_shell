...
define(['d3','utils','underscore'],function(d3,util,_){
    ...
    var nodeCommands = {
        ...
        "draw" : function(globalData,values){
            drawNodeAndContext(globalData,globalData.shell.cwd);
        },
    ...
    //--------------------
    //Utils
    var drawNodeAndContext = function(globalData,focusNode){
        var colWidth = globalData.calcWidth(globalData.usableWidth,columnNames.length),
            halfWidth = globalData.halfWidth(),
            nodeTextHeight = 20,
            nodeTextSeparator = 2,
            //get the data:
            nodeText = globalData.shell.getListsFromNode(focusNode,['id','name','values','tags','annotations']),
            childrenData = _.keys(focusNode.children).map(function(d){
                return this.allNodes[d];
            },globalData.shell),
            parentsData = _.keys(focusNode.parents).map(function(d){
                return this.allNodes[d];
            },globalData.shell),            
            //if necessary, init the containers
            mainContainer = util.selectOrShare('mainContainer',undefined,d3),
            //draw the node
            node = mainContainer.selectAll(".node").data([focusNode],function(d){ return d.id; }),
            //place in the centre
        ...
        //draw the nodes parents and children
        var parents = drawGroup(globalData,mainContainer, "parent", parentsData, (globalData.halfWidth() - (colWidth * 2)), colWidth),
            children = drawGroup(globalData,mainContainer, "child", childrenData, (globalData.halfWidth() + colWidth), colWidth);
